...
void create_cell_types_v1( void )
{
	// set the random seed 
	...
	*/ 
	...
	Cell_Definition* pCD = find_cell_definition( "leader cell"); 
	pCD->functions.update_phenotype = NULL; 
	// pCD->functions.update_phenotype = ecm_update_from_cell_motility_vector; 
	pCD->functions.custom_cell_rule = ecm_update_from_cell_motility_vector; 

	pCD->parameters.o2_proliferation_saturation = 38; 
	pCD->parameters.o2_reference = 38; 

    //-----------------------
	Cell_Definition* pCD2 = find_cell_definition( "follower cell"); 
    pCD2->functions.update_phenotype = NULL;// follower_cell_phenotype_model;
...
{	
    ...
    Cell_Definition* pCD_leader = find_cell_definition( "leader cell"); 
    Cell_Definition* pCD_follower = find_cell_definition( "follower cell"); 
	...
	if (parameters.ints("march_unit_test_setup") == 0)
	{
		...
		{
			Cell* pC;
	        // Cell_Definition* pCD = find_cell_definition( "leader_cell"); 
			// pC = create_cell(leader_cell);
			pC = create_cell( *pCD_leader );
		...
		else if(parameters.strings("cell_setup") == "lesion")
		{
            ...
            std::cout << "          lesion; tumor_radius= " << tumor_radius << std::endl;
			...
			while( y < tumor_radius )
			{
				...
				while( x < x_outer )
				{
					...
					{ 
			            ...
			            pCell = create_cell( *pCD_leader );
                    }
					else
					{ 
                        // pCell = create_cell(leader_cell); 
			            pCell = create_cell( *pCD_follower );
                    }
...
void setup_extracellular_matrix( void )
{
	...
	// if(parameters.bools("unit_test_setup") == 1)  // rwh: wth? due to "bools", not "ints" --> ERROR : Unknown parameter for operator(): unit_test_setup  
	// {
